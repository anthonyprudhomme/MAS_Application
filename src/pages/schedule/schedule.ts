...
export class SchedulePage {
  ...
  months = ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', "Sep", 'Oct', 'Noc', 'Dec'];
    ...
    this.platform.ready().then(() => {
      ...
      this.http.get('http://52.56.35.31:8088/getData', {}, {})
        .then(data => {
          ...
          console.log(jsonData.events[0].event.type_of_event); // data received by server
        })
        .catch(error => {
          this.fillEventList(
            {
  ...
  fillEventList(jsonData) {
    ...
    for (var i = 0; i < jsonData.events.length; i++) {
      ...
      if (currentEvent != null) {
        ...
        if (currentEvent.event.score == null) {
          currentEvent.event.score = i;
        }
  ...
  sortByScore() {
    this.events.sort(function (a, b) {
      return a.event.score - b.event.score;
    });
  }
